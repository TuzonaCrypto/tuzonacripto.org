...
import { Image as ImageIcon, Video, Trash2, RefreshCw, Upload } from 'lucide-react';
...
const BusinessFormModal = ({ 
  ...
  const handleCryptoChange = (crypto) => {
    setNewBusiness(prev => {
      const currentCryptos = prev.acceptedCryptos || [];
      return { ...prev, acceptedCryptos: currentCryptos.includes(crypto) ? currentCryptos.filter(c => c !== crypto) : [...currentCryptos, crypto] };
    });
  };

  const handleImageUpload = async (event) => {
    if (!user) {
      toast({ title: "Error", description: "Debes estar autenticado para subir imágenes.", variant: "destructive"});
      return;
    }
    ...
    if (file.size > 5 * 1024 * 1024) { // 5MB limit
      toast({ title: "Archivo muy grande", description: "El tamaño máximo por imagen es de 5MB.", variant: "destructive"});
      return;
    }
    ...
    try {
      ...
      setNewBusiness(prev => ({ ...prev, images: [...(prev.images || []), publicURLData.publicUrl] }));
      toast({ title: "Imagen Subida", description: "La imagen se ha agregado correctamente." });
  ...
  const removeImage = async (indexToRemove) => {
    const imageToRemove = (newBusiness.images || [])[indexToRemove];
    if (imageToRemove.includes('supabase.co')) { // Only try to delete if it's a Supabase URL
      try {
        const filePath = imageToRemove.substring(imageToRemove.indexOf('business_images/') + 'business_images/'.length);
        await supabase.storage.from('business_images').remove([filePath]);
      } catch (error) {
        console.error("Error deleting image from storage:", error);
        toast({ title: "Error al eliminar imagen del almacenamiento", description: error.message, variant: "destructive"});
        // Decide if you want to proceed with removing from list even if storage deletion fails
      }
    }
        ...
        <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-32 overflow-y-auto p-2 bg-black/20 rounded-md">
              {cryptoCurrencies.map(crypto => (
                <label key={crypto} className="flex items-center space-x-2 p-2 rounded-md hover:bg-black/40 cursor-pointer">
                  <input type="checkbox" checked={(newBusiness.acceptedCryptos || []).includes(crypto)} onChange={() => handleCryptoChange(crypto)} className="form-checkbox h-4 w-4 text-red-600 bg-gray-700 border-gray-600 rounded focus:ring-red-500" disabled={isLoadingForm}/>
          ...
          <div>
            ...
            {(newBusiness.images || []).length < planLimits.maxImages && (
              <>
                <input type="file" accept="image/png, image/jpeg, image/gif, image/webp" onChange={handleImageUpload} ref={fileInputRef} className="hidden" disabled={isLoadingForm}/>
                <Button type="button" onClick={triggerFileInput} variant="outline" className="w-full border-red-500 text-red-400 hover:bg-red-500/10" disabled={isLoadingForm}>
